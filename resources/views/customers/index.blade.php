...
<script>
    ...
    var pseries = new Array({{count($exports)}});
        @forelse($exports as $export)
            @if($export->suma>$export->payed)
                pseries['{{$export->id}}']="{{$export->customer->pserie.$export->customer->pnumber}}";
            @endif
        @empty
        @endforelse
    ...
    function reloadn(){
        ...
        ans.forEach(function(item){
            var indices = [];
            var element = item;
            var idx = names.indexOf(element);
            while (idx != -1) {
                indices.push(idx);
                idx = names.indexOf(element, idx + 1);
            }
            indices.forEach(function(item1){
                bod+=trs[item1];
            });
        });
        tbody.innerHTML=bod;
    ...
    function reloadp() {
        ...
        ans.forEach(function(item){
            var indices = [];
            var element = item;
            var idx = pseries.indexOf(element);
            while (idx != -1) {
                indices.push(idx);
                idx = pseries.indexOf(element, idx + 1);
            }
            indices.forEach(function(item1){
                bod+=trs[item1];
            });
